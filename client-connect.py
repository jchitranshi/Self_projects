...
def choice(arg):
	...
	if client:
		temp="Game Round "+str(game_round)+your_choice
		d = temp.encode('ASCII')
		#print("#############33\n",d)
		client.send(d)
...
def receive_message_from_server(sck,m):
	global your_name,opponent_name,round_result,game_round
	global your_choice,opponent_choice,your_score,opponent_score

	while True:
		from_server=sck.recv(1024)

		if not from_server:
			break
		#print("from_server:",from_server)
		d = from_server.decode('ASCII') 
		#print("d------------------>",d)

		if d.startswith("welcome"):
			if d=="welcome1":
		...
		elif d.startswith("opponent_name$"):
			opponent_name=d.replace("opponent_name$","")
			lbl_opponent_name["text"]="Opponent: "+opponent_name
			top_frame.pack()
			middle_frame.pack()

			#we know two users are connected so the game is ready to start
			threading._start_new_thread(count_down,(game_timer,""))
			#threading.Thread(target=count_down,args=(game_timer," "))
			lbl_welcome.config(state=tk.DISABLED)
			lbl_line_server.config(state=tk.DISABLED)

		elif d.startswith("$opponent_choice"):
			#get the opponent choice from server
			opponent_choice=d.replace("$opponent_choice","")
...
def connect_to_server(name):
	global client,HOST,PORT,your_name
	try:
		#print("chit3")
		...
		#tk.messagebox.showerror(title="ERROR!!!",message="Cannot connect to host:"+HOST)
...
def connect():
	...
	if len(ent_name.get())<1:
		...
		#print("Ypur name: ",your_name)
		connect_to_server(str(your_name))
